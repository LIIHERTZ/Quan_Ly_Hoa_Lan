...
[ApiController]
[Route("api/cultivation-categories")]
[Authorize(Roles = "Admin")]
public class CultivationCategoriesController : ControllerBase
{
    private readonly IMediator _mediator;

    public CultivationCategoriesController(IMediator mediator)
    {
        _mediator = mediator;
    }
    ...
    {
        query.UseType(ArticleCategoryType.CULTIVATION);
    ...
    {
        return Ok(await _mediator.Send(new GetArticleCategoryTreeQuery(ArticleCategoryType.CULTIVATION)));
    }
    ...
    public async Task<IActionResult> GetById(Guid id)
    {
        return Ok(await _mediator.Send(
            new GetArticleCategoryByIdQuery(id, ArticleCategoryType.CULTIVATION)));
    }
    ...
    {
        command.Type = ArticleCategoryType.CULTIVATION;
        var id = await _mediator.Send(command);
    ...
    {
        await _mediator.Send(new DeleteArticleCategoryCommand(id, ArticleCategoryType.CULTIVATION));
        return NoContent();
    }
}
